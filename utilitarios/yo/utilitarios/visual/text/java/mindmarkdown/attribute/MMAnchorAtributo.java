...
//FUNCS
	public void applyStyle(MindMarkDocumento doc){
		...
		//END_TAG
			...
			//ESCAPED_TAG
				if(match.group("escapeEnd")!=null){
					MindMarkAtributo.styleEscape(doc,match.start("escapeEnd"),match.group("escapeEnd").length(),match.group("endTag").length());
				}else{
				//NON_ESCAPED_TAG
					if(match.group("nonEscapeEnd")!=null){
						MindMarkAtributo.styleNonEscape(doc,match.start("nonEscapeEnd"),match.group("nonEscapeEnd").length(),match.group("endTag").length());
					}
				//END_TAG
		...
		//FINISH
			...
			MindMarkAtributo.styleLine(doc,indexTagLeft,lengthTagLeft,atributo,indexTagRight,lengthTagRight);
		}
	}
}