...
public class MMCitationAtributo extends MindMarkdownAtributo{
//VAR GLOBAIS
	...
	public void applyStyle(MindMarkdownDocumento doc){
		...
		//TAG
			...
			for(int i=1;i<=MAX_LEVEL;i++){
				...
				if(MindMarkdownAtributo.isStyledSpecial(doc,match.start("tagLevel"+i)))break;		//JÁ FOI ESTILIZADO
			//ESCAPE
				if(match.group("escapeLevel"+i)!=null){
					MindMarkdownAtributo.styleEscape(doc,match.start("escapeLevel"+i),match.group("escapeLevel"+i).length(),match.group("tagLevel"+i).length());
				}
			//NON-ESCAPE
				if(match.group("nonEscapeLevel"+i)!=null){
					MindMarkdownAtributo.styleNonEscape(doc,match.start("nonEscapeLevel"+i),match.group("nonEscapeLevel"+i).length(),match.group("tagLevel"+i).length());
				}
		...
		//FINISH
			if(MindMarkdownAtributo.isStyledSpecial(doc,indexTag))continue;	//JÁ FOI ESTILIZADO
			...
			MindMarkdownAtributo.styleLine(doc,indexTag,lengthTag,atributo,indexText+lengthText,0);
		}
	}
