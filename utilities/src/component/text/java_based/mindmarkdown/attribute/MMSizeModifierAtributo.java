...
public class MMSizeModifierAtributo extends MindMarkdownAtributo{
//VAR GLOBAIS: SIZES
...
//FUNCS
	public void applyStyle(MindMarkdownDocumento doc){
		...
		//ESCAPED_TAG
				...
				if(MindMarkdownAtributo.isStyledSpecial(doc,match.start("tag")))continue;	//JÁ FOI ESTILIZADO
				MindMarkdownAtributo.styleEscape(doc,match.start("escape"),match.group("escape").length(),match.group("tag").length());
		//TAG
			}else{
				if(MindMarkdownAtributo.isStyledSpecial(doc,match.start("tag")))continue;	//JÁ FOI ESTILIZADO
			//NON_ESCAPED_TAG
				if(match.group("nonEscape")!=null){
					MindMarkdownAtributo.styleNonEscape(doc,match.start("nonEscape"),match.group("nonEscape").length(),match.group("tag").length());
				}
			//TAG
		...
		private void applyStyle(MindMarkdownDocumento doc,boolean hasHittedLimit,int sizeIndex,Match matchIndex,Match matchTexto){
			...
			if(!hasHittedLimit){
				MindMarkdownAtributo.styleText(doc,matchIndex.getIndex(),matchIndex.getLength(),atributo,endIndex,0);
			}else MindMarkdownAtributo.styleText(doc,matchIndex.getIndex(),0,atributo,endIndex,0);
		}
