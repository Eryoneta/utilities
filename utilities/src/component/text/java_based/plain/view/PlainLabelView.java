...
//EDITOR_KIT
	private PlainEditor editorKit;
//MAIN
	public PlainLabelView(Element elem,PlainEditor editor){
		super(elem);
		editorKit=editor;
	}
//FUNCS
...
@Override
	...
	//LETRAS ESCONDIDAS
		if(editorKit.isAllCharsVisible()){
			...
			final Font fonte=new Font(PlainEditor.DEFAULT_FONT.getFamily(),PlainEditor.DEFAULT_FONT.getStyle(),imagemEdit2D.getFont().getSize());
			...
			for(int i=0;i<texto.length();i++){
				...
				switch(letra){
					case ' ':				//ESPAÇO
						final int adjustSpaceX=(fontMetrics.stringWidth(PlainEditor.SPACE_SYMBOL)-fontMetricsDefault.stringWidth(PlainEditor.SPACE_SYMBOL))/2;
						imagemEdit2D.setColor(PlainEditor.SPECIAL_CHARACTERS_COLOR);
						imagemEdit2D.drawString(PlainEditor.SPACE_SYMBOL,local.x+adjustSpaceX,local.y);
					break;
					case '\t':				//TAB
					...
					case '\n':case '\r':	//ENTER/CARRIER
						final int adjustEnterX=(fontMetrics.stringWidth(PlainEditor.ENTER_SYMBOL)-fontMetricsDefault.stringWidth(PlainEditor.ENTER_SYMBOL))/2;
						imagemEdit2D.setColor(PlainEditor.SPECIAL_CHARACTERS_COLOR);
						imagemEdit2D.drawString(PlainEditor.ENTER_SYMBOL,local.x+adjustEnterX,local.y);
					break;
				}
			}
		}
